<template>
    <div>
        <div class="galleryTitle">
            <p>
                Revive los momentos mágicos año tras año en nuestro Álbum
                Fotográfico
            </p>
        </div>
        ...
        <div>
            <p class="galleryParagraph">
                Déjate envolver por la nostalgia y la alegría al revivir los
                momentos más mágicos de tu bebé a lo largo de los años. En
                nuestro Álbum Fotográfico, cada imagen es una ventana al pasado,
                donde podrás revivir sonrisas, abrazos y aventuras. Celebra el
                crecimiento de tu tesoro y disfruta de un recorrido lleno de
                amor y dulces recuerdos.
            </p>
        </div>
    </div>
...
export default {
    ...
    computed: {
        uniqueYears() {
            const years = this.multimediaData.map((image) =>
                new Date(image.FechaSubida).getFullYear()
            );
            return [...new Set(years)];
        },
        filteredImages() {
            if (this.selectedYear) {
                return this.multimediaData.filter(
                    (image) =>
                        new Date(image.FechaSubida).getFullYear() ===
