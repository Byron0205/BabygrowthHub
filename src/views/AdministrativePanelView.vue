    ...
    <div class="page-wrapper">
        <div class="container-adm-family">
            <div class="left-container">
                <p class="text-info-adm">
                    ¡Disfruta de la oportunidad de liderar y cuidar de tu
                    preciado grupo familiar en este espacio dedicado a su
                    bienestar y felicidad!
                </p>
                <div class="family-adm-group">
                    <div class="family-info">
                        <h2 class="title-adm-family">
                            Encargada/o del grupo familiar:
                        </h2>
                    ...
                    <div class="family-role">
                        <i
                            ...
                            style="color: #4f439a; font-weight: 600"
                        ></i>
                        <p class="rol-adm-family">
                            {{ encargadoPrincipalRole }}
                        </p>
                    </div>
                </div>
                <div
                    v-if="selectedMemberIndex !== null"
                    class="container-select-guess"
                >
                    <div class="select-guess-dash">
                        <p class="text-select-guess-dash">
                            Selecciona el invitado a modificar su rol
                        </p>
                    </div>
                    <div class="select-guess-border">
                        <p class="selected-user-rol">
                            {{
                                selectedMemberIndex !== null
                                    ? familyMembers[selectedMemberIndex].name
                                    : ""
                            }}
                        </p>
                        <select
...
export default {
    ...
    watch: {
        selectedMemberIndex(newValue) {
            this.selectedMemberRole =
                newValue !== null ? this.familyMembers[newValue].role : "";
        },
    ...
    methods: {
        fetchFamilyMembers(idBebe) {
            axios
                .get(`http://localhost:3000/adultosxbebe/${idBebe}`)
                .then((response) => {
                    ...
                    const encargadoPrincipalMember = this.familyMembers.find(
                        (member) => member.isEncargadoPrincipal
                    );
                    if (encargadoPrincipalMember) {
                        this.encargadoPrincipal = encargadoPrincipalMember.name;
                        this.encargadoPrincipalRole =
                            encargadoPrincipalMember.role;
                    } else {
        ...
        async updateRole() {
            if (this.selectedMemberIndex !== null) {
                ...
                const existingRoles = this.familyMembers.map(
                    (member) => member.role
                );
                const roleCount1 = existingRoles.filter(
                    (role) => role === "Madre"
                ).length;
                const roleCount2 = existingRoles.filter(
                    (role) => role === "Padre"
                ).length;

                if (
                    (newRole === "1" && roleCount1 >= 1) ||
                    (newRole === "2" && roleCount2 >= 1)
                ) {
                    console.error(
                        "Error: There can only be one member with role 'Madre' (1) and one member with role 'Padre' (2)."
                    );
                ...
                const { name, role } =
                    this.familyMembers[this.selectedMemberIndex];
                const IDAdulto =
                    this.familyMembers[this.selectedMemberIndex].IDAdulto;

                try {
                    await axios.put(
                        `http://localhost:3000/adultosxbebe/${IDBebe}`,
                        {
                            IDAdulto: IDAdulto,
                            NuevoRol: role,
                        }
                    ...
                    console.error(
                        "Error al modificar el rol en el backend:",
                        error
                    );
                }
            }
